...
class NeuralNetwork {
	...
	constructor(inputsNumber, activationFunction, derivateFunction, neuronsNumber, learningRate = 0.1) {
		...
		for (let i = 0; i < neuronsNumber; i++) {
			this.#hiddenNeurons.push(new Neuron(inputsNumber, activationFunction, this.#derivateFunction, learningRate));
		}

		// Create output neuron, which takes hidden neuron outputs as inputs
		this.#outputNeuron = new Neuron(neuronsNumber, activationFunction, this.#derivateFunction, learningRate);
	}
